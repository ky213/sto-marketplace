...
import { getSearchEntities, getEntities } from './user-setting.reducer';
...
export const UserSetting = (props: IUserSettingProps) => {
  ...
  const { userSettingList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="user-setting-heading">
        User Settings
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('iban')}>
                  Iban <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('ethAddress')}>
                  Eth Address <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('riskProfil')}>
                  Risk Profil <FontAwesomeIcon icon="sort" />
                </th>
            ...
            <tbody>
              {userSettingList.map((userSetting, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <TextFormat type="date" value={userSetting.dateOfBirth} format={APP_DATE_FORMAT} />
                  </td>
                  <td>{userSetting.nationality}</td>
                  <td>{userSetting.phoneNumber}</td>
                  <td>{userSetting.position}</td>
                  <td>{userSetting.address}</td>
                  <td>{userSetting.code}</td>
                  <td>{userSetting.city}</td>
                  <td>{userSetting.country}</td>
                  <td>{userSetting.iban}</td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${userSetting.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${userSetting.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
      ...
      <div className={userSettingList && userSettingList.length > 0 ? '' : 'd-none'}>
...
const mapStateToProps = ({ userSetting }: IRootState) => ({
  ...
  totalItems: userSetting.totalItems
});
